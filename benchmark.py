...
def main():
    ...
    parser.add_argument('-wn', help='Hardware name', dest='hardware_name', choices=["A100_GPU", "GC200_IPU", "H100_GPU","MI250_GPU","E4-GH200","E4-A2"], required=True)
    ...
    with energy_profiler() as measured_scope:
        ...
        try:
            ...
            if args.hardware == "cpu":
                ...
                # ML: The following throws the real error with AP5 on IPU, that is that overwriting train_step is not supported for Keras IPU extensions.
            ...
            if args.hardware == "ipu" and args.app_name == 'ap5':
            ...
            if main_process:
                ...
                print(f"   Performance mean epoch: {dataset_size_mb / np.mean(times)*scale_ds:.2f} MB/s")
                print(f"   Max epoch time: {np.max(times):.2f} s")
                print(f"   Performance max epoch: {dataset_size_mb / np.max(times)*scale_ds:.2f} MB/s")
